<template>
  <div
    class="usage-overtime mt-3 flex-col items-stretch"
    v-if="m"
  >
    <div class="usage-title mt-2 pt-1 text-grey text-18">
      Use over time for
    ...
    <div
      ...
      :style="{
        height: `${m.tablet.height / 2 }px`,
        width: `${m.tablet.width / 2 }px`,
      }"
    >
      <image-loader
        :height="m.tablet.height"
        :width="m.tablet.width"
        :url="`https://www.gstatic.com/onebox/dictionary/${m.tablet.url}`"
      />
    </div>
  </div>
</template>
